...
void TutorialGame::UpdateGame(float dt) {
	...
	if (lockedObject) {
		...
		if (CollisionDetection::ObjectIntersection(lockedObject, Floor, info)) {
			std::cout << "Åöµ½ÁË!!!!!!!!!!!!!!!!!" << std::endl;
		}
	}
...
GameObject* TutorialGame::AddSphereToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation, float radius, float mass) {
	...
	sphere->SetRenderObject(new RenderObject(body, Vector3(radius, radius, radius), sphereMesh, basicTex, characterShader));
...
GameObject* TutorialGame::AddPlayerToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation) {
	...
	character->SetRenderObject(new RenderObject(body, Vector3(1, 1, 1), charMesh, nullptr, characterShader));
...
GameObject* TutorialGame::AddEnemyToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation) {
	...
	reactphysics3d::Collider* collider = body->addCollider(shape, reactphysics3d::Transform::identity());
	character->SetPhysicsObject(body);
	character->SetRenderObject(new RenderObject(body, Vector3(1, 1, 1), enemyMesh, nullptr, characterShader));
	character->GetRenderObject()->SetColour(Vector4(0, 1, 0, 1));
...
GameObject* TutorialGame::AddEmitterToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation) {
	...
	OGLShader* newShader = new OGLShader("character.vert", "character.frag", "pointGeom.glsl");
...
GameObject* TutorialGame::AddBonusToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation) {
	...
	apple->SetRenderObject(new RenderObject(body, Vector3(2, 2, 2), bonusMesh, nullptr, characterShader));
...
StateGameObject* TutorialGame::AddStateObjectToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation, vector<Vector3> testNodes) {
	...
	reactphysics3d::Collider* collider = body->addCollider(shape, reactphysics3d::Transform::identity());
	apple->SetPhysicsObject(body);
	apple->SetRenderObject(new RenderObject(body, Vector3(3, 5, 3), enemyMesh, nullptr, characterShader));
...
BTreeObject* TutorialGame::AddGooseToWorld(const reactphysics3d::Vector3& position, const reactphysics3d::Quaternion& orientation, vector<Vector3> testNodes) {
	...
	reactphysics3d::Collider* collider = body->addCollider(shape, reactphysics3d::Transform::identity());
	apple->SetPhysicsObject(body);
	apple->SetRenderObject(new RenderObject(body, Vector3(2, 2, 2), gooseMesh, nullptr, characterShader));
...
void TutorialGame::InitDefaultFloor() {
	...
	Floor=AddFloorToWorld(reactphysics3d::Vector3( 0, -20, -400), reactphysics3d::Quaternion::identity(), reactphysics3d::Vector3(100, 2, 75));
}
	...
	if (inSelectionMode) {
		...
		if (Window::GetMouse()->ButtonPressed(NCL::MouseButtons::LEFT)) {
			...
			if (closestCollision->isHit) {
				...
				PaintSpray(closestCollision->hitPos);
...
/// PaintSparySystem
/// </summary>
void TutorialGame::PaintSpray(Vector3 explosionPos) {
	if (world->painted.size() <= paintMax-1) {
		world->painted.push_back(explosionPos);
	}
	else {
		if (paintcount < world->painted.size() - 1) {
			world->painted[paintcount] = explosionPos;
			paintcount++;
		}
		else {
			world->painted[paintcount] = explosionPos;
